...
public class TopAndroidClient {
     ...
     * @param userId 不能为null，通过userId确定需要刷新哪个access token。如果对应的access
     *            token不存在会抛出IllegalArgumentException
     * @param listener 不能为null，处理刷新结果的回调函数。
     * @param async 是否是异步执行刷新操作
     ...
     * 调用浏览器显示授权页面。
     * 
     * @param activity 通过{@code Intent}方式在浏览器上打开授权页面的{@code Activity}，不能为null
     */
    public void authorize(Activity activity) {
        if (activity == null) {
            throw new IllegalArgumentException("activity must not null.");
        }
        String str = getAuthorizeUrl();
     ...
     * 
     * @see TopApiListener
     * @param params 系统及业务参数
     * @param userId 需要使用哪个用户授权的access token来调用api，当API不需要session key时此参数可以为null
     * @param listener api调用回调处理监听器，不能为null
     * @param async true:异步调用；false:同步调用。Android 3.0以后会限制在UI主线程中同步访问网络，使用同步方式需谨慎
     * @throws IllegalArgumentException 当参数<code>params</code>或
     *             <code>listener</code>为null时
     */
     ...
     * @param ql
     * @param userId 需要使用哪个用户授权的access token来调用api，当API不需要session key时此参数可以为null
     * @param listener tql调用回调处理监听器，不能为null
     * @param async true:异步调用；false:同步调用。Android 3.0以后会限制在UI主线程中同步访问网络，使用同步方式需谨慎
     * @throws IllegalArgumentException 当参数<code>ql</code>或<code>listener</code>
     *             为null时
     */
        ...
        if (tokenFiles != null && tokenFiles.length > 0) {
            for (File tokenFile : tokenFiles) {
                ...
                try {
                    ...
                    Long userId = TOPUtils.getUserIdFromAccessToken(token);
                    if (userId != null) {
    ...
    public void addAccessToken(AccessToken accessToken) throws IOException {
        Long userId = TOPUtils.getUserIdFromAccessToken(accessToken);
        if (userId == null) {
            return;
        }
